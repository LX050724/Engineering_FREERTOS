...
void MouseKeyControlProcess(Mouse *mouse, Key *key)  
{
	...
	/***********************************************SHIFT控制,取弹位置切换**************************************************/
	if (key->v & Key_Shift) //第一排和第二排，操作手会有视角切换（第二排时图传略微向前）
	{
		if (Shiftflag==0)
		{
			if (GetMod == 1)
...
//		}
	...
	if (key->v & Key_E)  //shift+Q处于抓取状态，不论如何必须按下一次E才能下岛
	{
		if (Eflag == 0)
		{
			Eflag = 1;
	...
	if ((mouse->press_l) || (ML_Auto_flag == 1)) //此段所有时间参数都为0.1ms
	{
		if(GetMod!=0)
		{
			if (ML_Auto_flag == 0)
			...
			if (ML_Auto_flag == 1)
			{
				...
				else if (GetMod == 1)
				{
					if ((TIM7->CNT) > 4000)HAL_GPIO_WritePin(Q4_GPIO_Port, Q4_Pin, GPIO_PIN_SET);//夹
					if ((TIM7->CNT) > 6000)HAL_GPIO_WritePin(Q3_GPIO_Port, Q3_Pin, GPIO_PIN_SET);//抬
					if ((TIM7->CNT) > 16000)Location_mode_Sent(630);  //电推杆出去的位置
